...
<body>
  ...
  <p class="banner">RSA-2.0 family. Lenses: zero, tazel, vyrn, uv, candle, rosetta, zen, chaos, balance. Ultraviolet light analysis (uv) is synthetic. Candlelight analysis (candle) is synthetic. Ink isolates writing; page isolates parchment. Balance never invents marks. Author: Aziel Eliab.</p>
...
function openapiSpec(request) {
  ...
  return {
    ...
    paths: {
      ...
      "/v1/modes": { get: { operationId: "spectrallock_modes", summary: "List SpectralLock lenses (zero, tazel, vyrn, uv, candle, rosetta, zen, chaos, balance).", responses: { "200": { description: "modes" } } } },
      ...
      "/v1/overlay": {
        post: {
          ...
          requestBody: { required: true, content: { "application/json": { schema: { type: "object", properties: { b64: { type: "string" }, mode: { type: "string", enum: ["zero","tazel","vyrn","uv","candle","rosetta","zen","chaos","balance","ultraviolet","uv-light","uvsa","candlelight","candle-light"] }, lens: { type: "string" }, lenses: { type: "array", items: { type: "string" } }, target: { type: "string", enum: ["ink", "page"] } }, required: ["b64"] } } } },
...
async function handleRuntime(request, url) {
  ...
  if (path === "/v1/health" && request.method === "GET") {
    return json({
      ...
      lenses: ["zero", "tazel", "vyrn", "uv", "candle", "rosetta", "zen", "chaos", "balance"],
      targets: ["ink", "page"],
      synthetic_uv: true,
      synthetic_candle: true,
