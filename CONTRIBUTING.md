...
10. Unredact / lift-overlay never invents letters. Opaque sanitized rewrite
    with no leftover container bytes refuses (`SL-UNREDACT-OPAQUE`). Leftover
    / historical bytes recovery is a container read (including stale `/Page`
    graphs, xref/ObjStm, after-EOF), not a guess from a black box. OCR only
    after structural recovery; never reconstruct covered letters from
    context. Heatmaps are not transcripts.

## Where to change things

- Engines / formulas: `spectrallock/engine.py`
- Unredact / leftover-bytes: `spectrallock/unredact.py`
- Deep historical PDF recovery: `spectrallock/pdfhist.py`
